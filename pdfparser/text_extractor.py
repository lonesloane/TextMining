...
class PDFTextExtractor:
        ...
        try:
            ...
            if doc.is_extractable:
                ...
                for page_text, page_cells in pages:
                    self.logger.info("="*20)
                    self.logger.info("Validating page")
                    self.logger.info("="*20)
                    validated_txt, _continue = self.validate(page_text, page_cells)
                    if not _continue:
                        break
    ...
    def _parse_pages(self, doc):
        ...
        for i, page in enumerate(PDFPage.create_pages(doc)):
            ...
            if _log_level > 2:
                self.logger.debug('\n'+'-'*50)
                self.logger.debug('Processing page {i}'.format(i=i))
    ...
    def parse_page_layout(self, lt_objs, reverse=True):
        ...
        page_text = sorted([(key, value) for (key, value) in page_text.items()], reverse=reverse)
    ...
    def validate(self, page_txt, page_cells):
        """
        ...
        if PDFPageFilter.is_cover(page_txt):
            ...
            return '', False
        self.logger.debug('Before table filtering, length of page text:{len}'.format(len=len(page_txt)))
        PDFPageFilter.ignore_tables_content(page_txt, page_cells)
        self.logger.debug('After table filtering, length of page text:{len}'.format(len=len(page_txt)))

        self.logger.debug('Exit page validation')
        txt = re_assemble_text(page_txt)
        return txt, True


def re_assemble_text(page_txt):
    txt = [(-coord[1], -coord[0], str_array) for coord, str_array in page_txt.items()]
    txt = sorted(txt)
    logging.getLogger('summarizer').debug('-'*20)
    logging.getLogger('summarizer').debug('Sorted page text:')
    logging.getLogger('summarizer').debug('-'*20)
    for elem in txt:
        logging.getLogger('summarizer').debug('{elem}'.format(elem=elem))
    return ''.join([str_array for _, _, str_array in txt])
